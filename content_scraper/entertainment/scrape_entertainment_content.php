...
$stmt = "SELECT item_url FROM rssingest WHERE category_id = '3' AND source = 'Philippine Star' AND `update` IS NULL AND fetch_date >= CURDATE()";
...
$html2 = mysqli_real_escape_string($connection, $stmt); 
	...
	foreach($url as $row){
		...
		VALUES ('3', '" . $file . "','" . html_entity_decode($title) . "', '" . $image . "', '" .html_entity_decode($description). "', '" .html_entity_decode($content) . "', 'Philippine Star')");
...
function clean($content) {
  ...
  $content = str_replace('Entertainment ( Article MRec ),', '', $content);
...
function clean2($description) {
 ...
 $description = str_replace('</p>', '', $description);
  ...
  $description = str_replace('Entertainment ( Article MRec ),', '', $description);
