...
header("Content-Type: text/html; charset=utf-8");
	...
	$stmt = "SELECT item_url FROM rssingest WHERE category_id = '2' AND fetch_date >= CURDATE()"; 
	...
	foreach($url as $row){
		...
		echo $image = imageSrc($image);
		$content = getContent($html);
		$content = clean($content);
		$description = getContent($html);
		$description = clean2($description);
		$connection->query("INSERT INTO rss_content3(category_id, item_url, headline, image, description, content, source) 
		VALUES ('2', '" . $file . "','" . $title . "', '" . $image . "', '" .$description. "', '" .$content. "', 'Philippine Star')");
	...
	function getContent($html)
{
  ...
  $title = str_replace('â€“', '', $title);   return $title;
