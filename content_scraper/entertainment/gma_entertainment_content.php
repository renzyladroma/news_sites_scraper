...
$stmt = "SELECT item_url FROM rssingest WHERE source = 'GMA' AND category_id = '3' AND fetch_date >= CURDATE() AND `update` IS NULL"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		VALUES ('3', '" . $file . "','" . addslashes($title) . "', '" . $image . "', '" .addslashes($description). "', '" .addslashes($content)."', 'GMA')");
