...
$stmt = "SELECT item_url, category_id FROM rssingest WHERE source = 'GMA' AND fetch_date >= CURDATE() AND `update` IS NULL"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		if($result_article->num_rows < 1){
			...
			VALUES ('$category_id', '" . $file . "','" . addslashes($title) . "', '" . $image . "', '" .addslashes($description). "', '" .addslashes($content)."', 'GMA')");
