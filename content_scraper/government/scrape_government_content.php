...
$stmt = "SELECT item_url FROM rssingest WHERE category_id = '5' AND source = 'Philippine Star' AND `update` IS NULL AND fetch_date >= CURDATE()";
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		VALUES ('5', '" . $file . "','" . $title . "', '" . $image . "', '" .$description. "', '" .$content."', 'Philippine Star')");
		
	}
	
function clean($content) {
	$content = str_replace('Nation ( Article MRec ), pagematch: 1, sectionmatch: 1', '', $content);
	$content = str_replace('GS_googleAddAdSenseService("ca-pub-1876439796539993");', '', $content);
	$content = str_replace('GS_googleEnableAllServices();', '', $content);
	$content = str_replace('GA_googleAddSlot("ca-pub-1876439796539993", "PStar_Nation_Medallion_300x250");', '', $content);
	$content = str_replace('GA_googleFetchAds();', '', $content);
	$content = str_replace('GA_googleFillSlot("PStar_Nation_Medallion_300x250");', '', $content);
