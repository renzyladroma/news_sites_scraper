...
$stmt = "SELECT item_url, category_id FROM rssingest2 WHERE source = 'Manila Times' AND fetch_date >= CURDATE()"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		//Check Article
		$check_article_sql = "SELECT headline FROM rss_content3 where headline = '".addslashes($title)."'";
