...
$stmt = "SELECT item_url, category_id FROM rssingest WHERE source = 'Inquirer' AND fetch_date >= CURDATE()"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		if($result_article->num_rows < 1){
			...
			VALUES ('$category_id', '" . $file . "','" . addslashes($title) . "', '" . $image . "', '" .addslashes($description). "', '" .addslashes($content)."', 'Inquirer')");
...
function getContent($html){
    ...
    $split = explode('<div id="art_body_wrap">',$html);

    if(count($split) > 1)
    {
   
        $end = explode('Subscribe to', $split[1]);
		$end2 = $end[0];
