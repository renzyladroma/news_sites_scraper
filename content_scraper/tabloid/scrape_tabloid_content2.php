...
<?php
	...
	$stmt = "SELECT rssingest.item_url FROM abaka_db.rssingest WHERE rssingest.category_id = '1' AND rssingest.fetch_date >= CURDATE() AND rssingest.update IS NULL and rssingest.source = 'SunStar'";
	$url = $con->query($stmt);
	...
	foreach($url as $row){
		...
		$con->query("INSERT INTO abaka_db.rss_content(category_id, item_url, headline,image, description, content) VALUES ('1', '" . $file . "','" . $title . "', '" . $image . "', '" . $description . "', '" . $content . "')");
