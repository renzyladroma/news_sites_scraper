...
$stmt = "SELECT item_url, category_id FROM rssingest WHERE source = 'PAGEONE' AND fetch_date >= CURDATE()"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		if($result_article->num_rows < 1){
			...
			VALUES ('$category_id', '" . $file . "','" . addslashes($title) . "', '" . $image . "', '" .addslashes($description). "', '" .addslashes($content)."', 'PAGEONE')");
...
function getContent($html){
    ...
    $split = explode('<div class="td-post-content">',$html);
    ...
    if(count($split) > 1)
    {
		...
		$end2 = $end[0];
		$tags = array("img", "style", "a", "script", "span", "div");
