...
$stmt = "SELECT item_url, category_id FROM rssingest WHERE source = 'Manila Bulletin' AND fetch_date >= CURDATE()"; //AND `update` IS NULL 
...
$html2 = mysqli_real_escape_string($connection, $stmt);
	...
	foreach($url as $row){
		...
		if($result_article->num_rows < 1){
			...
			VALUES ('$category_id', '" . $file . "','" . addslashes($title) . "', '" . $image . "', '" .addslashes($description). "', '" .addslashes($content)."', 'Manila Bulletin')");
...
function getContent($html){
    ...
    $split = explode('<main id="tm-content" class="tm-content">',$html);

    if(count($split) > 1)
    {
   
        $end = explode('<p>Tags', $split[1]);
		$end2 = $end[0];
		$tags = array("img", "style", "a", "script", "span", "div", "h1", "h2", "strong");
